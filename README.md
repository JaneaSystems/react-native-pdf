...
Currently, Windows support is parital. The rn-fetch-blob module is not yet avaiable on Windows, only loading bundled PDFs is supported.
...
- Right-click main application project > Add > Reference...
  ...
  - If running 0.62, aslo select `RCTPdf`
...
To add a `test.pdf` like in the example add add:
```
